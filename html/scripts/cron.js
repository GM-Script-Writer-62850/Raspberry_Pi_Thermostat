	...
	httpRequest.onreadystatechange = function(){
		if(httpRequest.readyState==4){
			if(httpRequest.status==200){
				...
				alert('A '+httpRequest.status+' error occuded while fetching events!');
		}
	};
...
function saveEvents(){
	...
	httpRequest.onreadystatechange = function(){
		if(httpRequest.readyState==4){
			if(httpRequest.status==200)
				alert(httpRequest.responseText=="OK"?'Saved!':httpRequest.responseText);
			else
				alert("A "+httpRequest.status+" error occured while saving.");
		}
	};
	httpRequest.open('POST', 'cron.php');
	cron="cron="+encodeURIComponent(JSON.stringify(cron));
